    ...
    <div className="scroll-smooth bg-white text-black">
      ...
      >
        <div className="relative z-10 px-4">
          ...
          <blockquote className="text-lg italic max-w-2xl mx-auto text-white drop-shadow">
            {njOpenwords}
          </blockquote>
        </div>
      </header>
      ...
      <nav className="flex justify-center gap-6 bg-green-200 py-4 sticky top-0 shadow z-20">
        <a href="#where" className="text-green-900 font-semibold hover:underline">Where?</a>
        <a href="#history" className="text-green-900 font-semibold hover:underline">History</a>
        <a href="#explore" className="text-green-900 font-semibold hover:underline">Explore the City</a>
      </nav>
      ...
      <main className="px-6 md:px-16 py-10 space-y-16 max-w-4xl mx-auto">
        <section id="where">
          ...
          <p className="mb-6">
            Nanjing (南京) is located in eastern China, the capital city of Jiangsu (江苏) province.
            It sits on the Yangtze River and is a key city in the Yangtze River Delta region (长江三角洲). As the first city you reach after crossing the Yangtze River in East China, Nanjing is a vital transportation hub: Nanjing South Railway Station (南京南站) is one of the largest in Asia, linking the city to Beijing, Shanghai, and beyond.
          </p>
          <p className="mb-6">
          It is not just "somewhere near Shanghai (上海)"<del> as all people in East China introduce their hometown to foreigners </del>. However, it is very close, just a one-hour high-speed train ride away, so remember to visit Nanjing (and other cities in Jiangsu) when you are in Shanghai!
          </p>
...
<p>
  Fun fact: Nanjing's history as China's capital is longer than the history of the United States as a nation.
        ...
        <section id="explore">
          ...
          <p>
            Discover scenic spots like the Sun Yat-sen Mausoleum, Purple Mountain,
            Confucius Temple, and the ancient city wall. Don't forget to enjoy the famous salted duck! (To be continued...)
          </p>
        </section>
      </main>
    </div>
...
export default HometownPage;
    ...
    //   <header className="bg-gray-200 py-10 text-center px-4">
    //     <h1 className="text-4xl font-bold mb-4">{hometownMeta.title}</h1>
    //     <p className="text-lg max-w-2xl mx-auto">{hometownMeta.description}</p>
    //   </header>

    //   <main className="px-6 md:px-12 py-10">
    //     {/* 💡 Opening Words Section */}
    //     <blockquote className="relative bg-white shadow-md border-l-4 border-blue-500 px-6 py-4 mb-10 text-xl italic text-gray-700">
    //       {njOpenwords}
    //     </blockquote>

    //     {/* 📚 Main Content Section */}
    //     <section className="prose lg:prose-xl max-w-4xl mx-auto">
    //       {hometownContent}
    //     </section>
    //   </main>
    // </div>
